...
@section('content')
  ...
  <form action="{{route('region.update', ['region' => $region->id])}}" method="post">
    ...
    <div class="mb-2">
      ...
      <input type="text" name="title" value="{{old('title', $region->title)}}" id="title" class="form-control @error('title') is-invalid @enderror">
      @error('title')
    ...
    <div class="mb-2">
      ...
      <input type="text" name="slug" value="{{old('slug', $region->slug)}}" id="slug" class="form-control @error('slug') is-invalid @enderror">
      @error('slug')
        <div class="text-danger">{{$message}}</div>
      @enderror
    </div>
